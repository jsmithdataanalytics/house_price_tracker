...
def send_email():
    ...
    __send_email(body=str(table))


def __send_email(body: str):

    if not date.today().weekday():
        ...
        for recipient in email_connection.extra_dejson['recipients']:
            ...
            msg['To'] = recipient
            msg['Subject'] = 'Weekly House Price Report'
...
def analyse_prices():
    ...
    grouped = listings.groupby(['year', 'week'])
    grouped.mean().join(grouped.count(), rsuffix='_count')
    prices_by_week = grouped.mean().join(grouped.count(), rsuffix='_count')
    prices_by_week_2020 = prices_by_week.loc[2020][['listing_price', 'listing_price_count']]
